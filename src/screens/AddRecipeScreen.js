...
export default function AddRecipeScreen({ navigation }) {
  ...
  // Charge les options personnalisées de l'utilisateur
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    (async () => {
  ...
  const onSave = async () => {
    if (!validate()) return;
    if (!user) return notify('Erreur', 'Vous devez être connecté.');
    ...
    notify('Recette ajoutée', 'Votre recette a bien été enregistrée.');
    navigation.goBack();
  };
